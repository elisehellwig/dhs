...
plotDHS <- function(df, variable, region, colorpalette='PuBuGn', reverse=FALSE, point_size=0.5) {
	...
	#datacols <- the columns that contain the data you want to plot (basically everything but lat/long)

	#getting only the continent and variable of interest
	crows <- which(df[,'contcode']==region)
	dfc <- df[crows, ]
	...
	legend("bottomleft", title=paste(variable, 'in', region), legend=names(attr(colcode, "table")), fill=attr(colcode, "palette"), bty="n") ​
